...
class Circular extends CI_Controller {
    ...
    public function index() {
        ...
        $config['total_rows'] = $this->CircularPublic_model->count_public();
        ...
        $data['circular_new'] = $this->CircularPublic_model->fetch_circular($config['per_page'], $page);
        $data['hoDept'] = $this->CircularPrivate_model->getAllDepartments();
        $data['pagination'] = $this->pagination->create_links();
        $data['title'] = "Janata Bank PLC.";
        $data['page_details'] = $this->About_model->GetPageInformation('Circular');
        ...
        $this->load->view('public_circular', $data);
    }

    public function download_files($circular_id) {
        $this->load->helper('download');
        $fileinfo = $this->CircularPublic_model->download($circular_id);
        $file = file_get_contents('assets/file/circular/' . $fileinfo['circular_pdf_link']);
        $name = $this->CircularPublic_model->pdfname($circular_id);
    ...
    public function searchBy_no() {
        ...
        if (isset($circular_no) and !empty($circular_no)) {
            ...
            $data['title'] = "Janata Bank PLC.";
            $data['circular_new'] = $this->CircularPublic_model->searchBy_no($circular_no);
    ...
    public function searchBy_title() {
        ...
        if (isset($circular_title) and !empty($circular_title)) {
            ...
            $data['title'] = "Janata Bank PLC.";
            $data['circular_new'] = $this->CircularPublic_model->searchBy_title($circular_title);
            $this->load->view('circular_search', $data);
        } else {
            redirect('/page404');
        }
    ...
    public function searchBy_dept() {
        ...
        if (isset($circular_department) and !empty($circular_department)) {
            ...
            $data['title'] = "Janata Bank PLC.";
            $data['circular_new'] = $this->CircularPublic_model->searchBy_dept($circular_department);
            $this->load->view('circular_search', $data);
        } else {
            redirect('/page404');
        }
    ...
    public function searchBy_type() {
        ...
        if (isset($circular_type) and !empty($circular_type)) {
            ...
            $data['title'] = "Janata Bank PLC.";
            $data['circular_new'] = $this->CircularPublic_model->searchBy_type($circular_type);
            $this->load->view('circular_search', $data);
        } else {
            redirect('/page404');
        }
    ...
    public function searchBy_date() {
        ...
        if (isset($circular_fromdate) and !empty($circular_fromdate)) {
            ...
            $data['title'] = "Janata Bank PLC.";
            $data['circular_new'] = $this->CircularPublic_model->searchBy_date($circular_fromdate, $circular_todate);
            $this->load->view('circular_search', $data);
        } else {
            redirect('/page404');
        }
     ...
     public function pdfIframeView($circular_id) {
        if (!isset($_SERVER['HTTP_REFERER'])) {
            ...
            $data['title'] = "Janata Bank PLC.";
            $fileinfo = $this->CircularPublic_model->pdfname_files($circular_id);
            //print_r($fileinfo); 
