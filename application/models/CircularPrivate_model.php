...
class CircularPrivate_model extends CI_Model {
    ...
    public function codeToDept($circular_department) {
        $query = "SELECT office_name from jbl_ho_department WHERE office_code='$circular_department' ";
        $runQuery = $this->db->query($query);
        $deptName = $runQuery->row();
        return $deptName->office_name;
    }
