<html>
    ...
    <body>
        ...
        ?>
        <p><?php echo anchor("admin/promotion/addPromotion", '<i class="fa fa-plus"></i> Add Promotion', 'class="btn btn-primary"'); ?></p>
        <?php
        if (count($promotion_all)) {

            echo "<table id='dataTable' class='table table-striped table-bordered table-responsive-sm'>";
            echo "<thead><tr>";
            echo "<th>Promotion ID</th>
		  <th>Title of the Promotion</th>
		  <th>File Name</th>
		  <th>Status</th>
		  <th>Actions</th>";
            echo "</tr></thead>";

            foreach ($promotion_all as $key => $list) {
                ...
                echo '<td  class="center">' . anchor('admin/promotion/edit/' . $list['id'], '<i class="icon-edit icon-white"></i> Edit', 'class="btn btn-info"') . ' ' .
                anchor('admin/promotion/delete/' . $list['id'], '<i class="icon-trash icon-white"></i> Delete', 'class="btn btn-danger"') . '</td>';
