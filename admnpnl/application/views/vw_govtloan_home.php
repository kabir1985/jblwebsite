<html>
    ...
    <body>
        ...
        <?php
        if (!empty($govtloan_all)) {
            ?>
            <p align="right"><a href="<?php echo base_url(); ?>index.php/admin/govtloan/download_success_pdf_all">DOWNLOAD ALL APLICANTS' LIST</a></p>
            <?php
        }
        if ($this->session->flashdata('message')) {
        ...
        if (count($govtloan_all)) {
		  ...
		  <th>Action</th>";
            echo "</tr></thead>";
            foreach ($govtloan_all as $key => $list) {
                //{
                 $url = base_url();
