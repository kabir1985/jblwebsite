<html>
    ...
    <body>
        ...
        if ($this->session->flashdata('message')) {
            echo '<div class="alert alert-success"><a data-dismiss="alert" class="close" "><span class="glyphicon glyphicon-remove"></span></a>' . $this->session->flashdata('message') . '</div>';
        }
        ?>
        <p><?php echo anchor("admin/ec/addEC", '<i class="fa fa-plus"></i> Add EC Member', 'class="btn btn-primary"'); ?></p>
        <?php
        if (count($ec)) {
		  ...
		  <th>Actions</th>";
            ...
            foreach ($ec as $key => $list) {
                ...
                echo '<td  class="center">' . anchor('admin/ec/edit/' . $list['id'], '<i class="icon-edit icon-white"></i> Edit', 'class="btn btn-info"') . ' ' .
                anchor('admin/ec/delete/' . $list['id'], '<i class="icon-trash icon-white"></i> Delete', 'class="btn btn-danger"') . '</td>';
