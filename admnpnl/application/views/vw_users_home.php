<html>
    <head>
        <link href="<?php echo base_url(); ?>assets/table/DataTables-1.11.2/css/jquery.dataTables.css" rel="stylesheet">
    </head>
    <body>
        <?php
        if ($this->session->flashdata('message')) {
            echo '<div class="alert alert-success"><button data-dismiss="alert" class="close" type="button"><i class="glyphicon glyphicon-remove"></i></button>' . $this->session->flashdata('message') . '</div>';
        }
        ?>

        <p><?php echo anchor("admin/users/addUser", '<i class="fa fa-plus"></i> Add User', 'class="btn btn-primary"'); ?></p>

        <?php
        if (count($admins)) {

            echo "<table id=dataTable class='table table-striped table-bordered table-responsive-sm'>";
            echo "<thead><tr>";
            echo "<th>ID</th>
		  <th>Username</th>
		  <th>Department</th>
		  <th>User Group</th>
		  <th>OTP</th>
		  <th>Status</th>
		  <th>Actions</th>";
            echo "</tr></thead>";
            foreach ($admins as $key => $list) {

                echo "<tr>";
                echo "<td>" . $list['id'] . "</td>";
                echo "<td>" . $list['username'] . "</td>";
                echo "<td>" . $list['department'] . "</td>";
                echo "<td>" . $list['user_group'] . "</td>";
				echo "<td>" . $list['otp'] . "</td>";
                echo "<td align='center'>" . $list['status'] . "</td>";

                echo '<td  class="center">' . anchor('admin/users/edit/' . $list['id'], '<i class="icon-edit icon-white"></i> Edit', 'class="btn btn-info"') . ' ' .
                anchor('admin/users/delete/' . $list['id'], '<i class="icon-trash icon-white"></i> Delete', 'class="btn btn-danger"') . '</td>';

                /* echo anchor('admin/admins/edit/'.$list['id'],'edit');
                  echo " | ";
                  echo anchor('admin/admins/delete/'.$list['id'],'delete','class="delete"'); */
                //echo '<a href="'.base_url().'index.php/admin/admins/delete/'.$list['id'].'" onclick=" return confirm("Are you sure to delete this user record?");" >delete</a>';

                echo "</tr>";
            }
            echo "</table>";
        }
        ?>
        <script type="text/javascript">
            $('.delete').click(function () {
                var answer = confirm('Delete user');
                return answer; // answer is a boolean
            });
        </script>

        <script type="text/javascript" src="<?php echo base_url(); ?>assets/table/DataTables-1.11.2/js/jquery.dataTables.js"></script>
        <script>
            $(document).ready(function () {
                $('#dataTable').DataTable({

                });
            });
        </script>

    </body>

</html>