<html>
    ...
    <body>
        ...
        ?>
        <p><?php echo anchor("admin/videos/addVideos", '<i class="fa fa-plus"></i> Add Video Album', 'class="btn btn-primary"'); ?></p>
        <?php
        if (count($video)) {
            echo "<table id='dataTable' class='table table-striped table-bordered table-esponsive-sm'>";
            echo "<thead><tr>";
            echo "<th>ID</th>
		  <th>Title</th>
		  <th>Embed Code</th>
		  ...
		  <th>Actions</th>";
            ...
            foreach ($video as $key => $list) {
                ...
                echo '<td  class="center">' . anchor('admin/videos/edit/' . $list['id'], '<i class="icon-edit icon-white"></i> Edit', 'class="btn btn-info"') . ' ' .
                anchor('admin/videos/delete/' . $list['id'], '<i class="icon-trash icon-white"></i> Delete', 'class="btn btn-danger"') . '</td>';
