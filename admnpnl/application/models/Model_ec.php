...
class Model_ec extends CI_Model {
    ...
    function getallEC() {
        /* $sql = 'select id,priority,name,designation,committee_status,status from executive_committee';
          $query = $this->db->query($sql);
          if ($query->num_rows > 0) {
          return $query->result_array();
          } */
        $data = array();
        $Q = $this->db->get('executive_committee');
        if ($Q->num_rows() > 0) {
    ...
    function getTopEC($id) {
        $data = array();
        $options = array('id' => $id);
        $Q = $this->db->get_where('executive_committee', $options, 1);
        if ($Q->num_rows() > 0) {
            $data = $Q->row_array();
        }
        $Q->free_result();
    ...
    function addEC() {
        ...
        $this->db->insert('executive_committee', $data);
    }

    function delete($id) {
        $data = array('status' => 'Inactive');
        $this->db->where('id', $id);
        $this->db->update('executive_committee', $data);
    }
    ...
    function get_designation() {
        $sql = "select designation from executive_committee";
        $query = $this->db->query($sql);
            $data[0] = '--Please Select--';
    ...
    function get_comstatus() {
        $sql = "select committee_status from executive_committee";
        $query = $this->db->query($sql);     
            $data[0] = '--Please Select--';
            $data['Chairman'] = 'Chairman';
            $data['Member'] = 'Member';
    ...
    function get_status() {
        $sql = "select status from executive_committee";
        $query = $this->db->query($sql);
            $data[0] = '--Please Select--';
            $data['Active'] = 'Active';
